 ...
 */
public class NotTender extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mInflate = View.inflate(getActivity(), R.layout.notfragment, null);
        return mInflate;
    }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        bebing = (RecyclerView) mInflate.findViewById(R.id.noe_render);
        bebing.setLayoutManager(new LinearLayoutManager(getActivity()));
        String url = "https://wuye.kylinlaw.com/case/lawyer/list?status=2&token="+token;
        initdata(url);
        mRefreshLayout = (MaterialRefreshLayout) mInflate.findViewById(R.id.not_mater);
        ...
        mRefreshLayout.setMaterialRefreshListener(new MaterialRefreshListener() {
            ...
            public void onRefreshLoadMore(MaterialRefreshLayout materialRefreshLayout) {
                if(mlist.size()>1){
                    size = mlist.size()-1;
                    String url = "https://wuye.kylinlaw.com/case/lawyer/list?status=2&token="+token+"&lastId="+(mlist.size()-1);
                    initdata(url);
                }else{
        ...
        tv = (TextView) mInflate.findViewById(R.id.noe_shangla);
    }
        ...
        HttpOkGo.okgoget(url, MyTenderBean.class, new HttpOkGo.okget<MyTenderBean>() {
            ...
            public void shuju(ArrayList<MyTenderBean> list) {
                ...
                if(body.size()>=1){
                    ...
                    if(p==1){
                        mlist.addAll(body);
                        adapter = new MyTenderAdapter(mlist,getActivity(),token,3);
